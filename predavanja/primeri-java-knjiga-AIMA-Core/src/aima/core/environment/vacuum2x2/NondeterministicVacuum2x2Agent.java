 ...
 * @author Andrew Brown
 */
public class NondeterministicVacuum2x2Agent extends AbstractAgent
{
  ...
  public NondeterministicVacuum2x2Agent( PerceptToStateFunction ptsFunction )
  {
   ...
   * 
   * @param ptsFunction
   *          a function which returns the problem state associated with a given
   *          Percept.
   */
  ...
  {
    // check if goal state
    Vacuum2x2EnvironmentState state = ( Vacuum2x2EnvironmentState ) this
        .getPerceptToStateFunction().getState( percept );
    if ( state.getLocationState( Vacuum2x2Environment.LOKACIJA_A1 ) == Vacuum2x2Environment.LocationState.Cisto
        && state.getLocationState( Vacuum2x2Environment.LOKACIJA_A2 ) == Vacuum2x2Environment.LocationState.Cisto
        && state.getLocationState( Vacuum2x2Environment.LOKACIJA_B1 ) == Vacuum2x2Environment.LocationState.Cisto
        && state.getLocationState( Vacuum2x2Environment.LOKACIJA_B2 ) == Vacuum2x2Environment.LocationState.Cisto )
    {
